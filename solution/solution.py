...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        gpt4o_response = result["gpt4o_response"].replace("\n", " ")
        mini_response = result["mini_response"].replace("\n", " ")

        prompt = prompt[:40]
        gpt4o_response = gpt4o_response[:40]
        mini_response = mini_response[:40]

        lines.append(
            f"{prompt:<42} "
            f"{gpt4o_response:<42} "
            f"{mini_response:<42} "
            f"{result['gpt4o_latency']:>12.3f} "
            f"{result['mini_latency']:>12.3f}"
        )
